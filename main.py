...
async def close_attendance():
    ...
    res = students_collection.update_many(
        {"$nor": [{"date": {"$gte": last_hour_iso, "$lt": current_time_iso}}]},
        {"$inc": {"missed": 1}},
    )
...
async def define_coords_id(data: Teacher):
    print("from put /register: ", data)
    teacher_collection.update_one(filter={}, update={"$set": dict(data)}, upsert=True)
    cursor = teacher_collection.find({})[0]
    print("cursor value: ", cursor)
    res = teacher_serializer(cursor)
    print("serialized teacher: ", res)
    if len(teacher_id) == 0:
        teacher_id.append(res["primaryID"])
        print("first if conditional happened and teacher_id: ", teacher_id)
    else:
        print("second if conditional happened and teacher_id before pop: ", teacher_id)
        teacher_id.pop()
        print("second if conditional happened and teacher_id after pop: ", teacher_id)
        teacher_id.append(res["primaryID"])
        print(
            "second if conditional happened and teacher_id with new value added: ",
            teacher_id,
        )
    return res
